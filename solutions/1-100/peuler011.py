...
for i in range(len(matrix)):
    for j in range(len(matrix[i])-4):
        if prod(matrix[i][j:j+4]) > max_product:
            max_product = prod(matrix[i][j:j+4]) 
#Check Columns
for i in range(len(matrix_transposed)):
    for j in range(len(matrix_transposed[i])-4):
        if prod(matrix_transposed[i][j:j+4]) > max_product:
            max_product = prod(matrix_transposed[i][j:j+4]) 
